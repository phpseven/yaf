...
final class Application {
	...
	public function __construct($config, $envrion = null){
        ...
        register_shutdown_function(function (){
            ...
            if(!empty($last_error)){
                \Core::log(\Core::jsonEncode($last_error) , 'error');
            }
            $time_use = round( (microtime(true)-APP_START_TIME)*1000, 2);
            \Core::log("request in {$time_use}ms \n\n", 'end');

            \Core::writeLog();
        });
		$this->_environ = ini_get('yaf.environ');
